...
class CrawlerPipeline:
    ...
    async def execute(self, query: str = "stars:>10000", max_repos: int = 5, page: int = 1, sort: str = "stars") -> List[Dict[str, Any]]:
        logger.info(f"Executing CrawlerPipeline (MongoDB Mode) with query='{query}', max_repos={max_repos}, page={page}, sort='{sort}'")
        ...
        if not raw_repos:
            logger.warning("No repositories returned from GitHub API.")
            return []
        ...
        for repo in raw_repos:
            try:
                ...
                logger.error(f"Error processing repo {repo.get('name')}: {e}")
